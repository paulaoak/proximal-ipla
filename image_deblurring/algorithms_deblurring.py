...
def pipgla(theta, w_init, H, y, sigma, lambdaaa, Dx, Dy, original, K=1000, h=0.01, method_tv = "dr"):
    ...
    for k in tqdm(range(K)):
        # Parameter updates 
        theta = theta - h * np.exp(theta) * np.mean(total_variation(w, Dx, Dy))/(Dw) + h  + np.sqrt(2*h/N)*np.random.normal(0, 1, 1)
...
def mypgd(theta, w_init, H, y, sigma, lambdaaa, Dx, Dy, original, K=1000, h=0.01, method_tv = "dr"):
    ...
    for k in tqdm(range(K)):
        # Parameter updates 
        theta = theta - h * np.exp(theta) * np.mean(total_variation(w, Dx, Dy))/(Dw) + h 
        theta_values[k] = theta
