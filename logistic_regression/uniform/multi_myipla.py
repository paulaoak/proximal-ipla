...
from logistic_regression.algorithms import pip_ula, sig, mypipla, mypgd, prox_pgd, proximal_map_laplace_iteration_total, proximal_map_laplace_iterative, proximal_map_laplace_approx, proximal_map_laplace_approx_total, proximal_map_uniform, proximal_map_uniform_my, pipgla, proximal_map_uniform_my_new

os.chdir(project_root)

output_file = "output_myipla_uniform.txt"
...
def run_algorithm_proximal(_):
    ...
    thetas_approx, _ = mypipla(proximal_map = proximal_map_uniform_my_new, th = np.array([[theta0]]), X = X0, N = n_particles, design_matrix = design_matrix, data = labels, K = n_iterations, gamma = 0.5, h = 0.005)
